...
func init() {
	m = New()
	m.AddFunc("dummy/copy", func(w io.Writer, r io.Reader, m *M, params map[string]string) error {
		io.Copy(w, r)
		return nil
	})
	m.AddFunc("dummy/nil", func(w io.Writer, r io.Reader, m *M, params map[string]string) error {
		return nil
	})
	m.AddFunc("dummy/err", func(w io.Writer, r io.Reader, m *M, params map[string]string) error {
		return errDummy
	})
	m.AddFunc("dummy/charset", func(w io.Writer, r io.Reader, m *M, params map[string]string) error {
		w.Write([]byte(params["charset"]))
		return nil
	})
	m.AddFunc("dummy/params", func(w io.Writer, r io.Reader, m *M, params map[string]string) error {
		return m.Minify(w, r, params["type"]+"/"+params["sub"], nil)
	})
	m.AddFunc("type/sub", func(w io.Writer, r io.Reader, m *M, params map[string]string) error {
		w.Write([]byte("type/sub"))
		return nil
	})
	m.AddFuncPattern(regexp.MustCompile("^type/.+$"), func(w io.Writer, r io.Reader, m *M, params map[string]string) error {
		w.Write([]byte("type/*"))
		return nil
	})
	m.AddFuncPattern(regexp.MustCompile("^.+/.+$"), func(w io.Writer, r io.Reader, m *M, params map[string]string) error {
		w.Write([]byte("*/*"))
		return nil
	})
}
...
func ExampleMinify_Custom() {
	m := New()
	m.AddFunc("text/plain", func(w io.Writer, r io.Reader, m *M, params map[string]string) error {
		// remove all spaces
