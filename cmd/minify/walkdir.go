...
func walkDir(fsys string, name string, d os.DirEntry, walkDirFn WalkDirFunc) error {
	...
	dirs, err := os.ReadDir(filepath.Join(fsys, name))
	if err != nil {
...
func WalkDir(fsys string, root string, fn WalkDirFunc) error {
	info, err := os.Stat(filepath.Join(fsys, root))
	if err != nil {
		err = fn(root, nil, err)
	} else {
